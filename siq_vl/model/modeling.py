...
class SiQ_VLProjector(PreTrainedModel):
    """
    SiQ-VL projector module (v2 - 2-Layer MLP with Residual + LayerNorm).

    This module projects the vision features into the language model embedding space.
    Architecture:
    1. Pixel Shuffle: compress vision features (L → L/r² tokens)
    2. Linear1: project from vision_dim*r² to text_dim
    3. GELU activation: nonlinearity for better alignment
    4. Linear2: refine within text embedding space
    5. LayerNorm: stabilize visual token distribution
    6. Residual: add Linear1 output for stability

    This 2-layer design allows learning nonlinear mappings between visual and
    language embedding manifolds, which is critical because these spaces have
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if module is self.linear1:
                ...
                # Small init for second layer to start near identity (with residual)
    ...
    def forward(self, x):
        ...
        # 2. 2-Layer MLP with residual connection
        residual = self.linear1(x)       # (B, L/r², text_dim)
        h = self.act(residual)            # GELU nonlinearity
        h = self.linear2(h)              # (B, L/r², text_dim)
...
class SiQ_VLTextModel(Qwen3ForCausalLM):
    """
    SiQ-VL text model based on Qwen3ForCausalLM.
    Updated: Use Qwen3ForCausalLM instead of Qwen2ForCausalLM for proper
    compatibility with Qwen3-0.6B weights (which have q_norm/k_norm and no bias).
    """
...
class SiQ_VLForCausalLM(SiQ_VLPreTrainedModel, GenerationMixin):
    ...
    def unfreez_text_model(self):
        ...
        self.text_model.train()  # Fixed: was eval(), should be train() for LoRA
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if past_key_values is None and pixel_values is not None:
            ...
            # Ensure vision embeddings are on the same device as text embeddings
            # (device_map="auto" may place vision and text models on different GPUs)
            vision_token_embeddings_flat = vision_token_embeddings_flat.to(token_embeddings.device)
...
def get_stage1_model_and_processor(
    ...
    """
    Get the initialized SiQ-VL model for stage 1 (multimodality projector alignment) pre-training.
    Updated defaults for Qwen3-0.6B + SigLIP2-so400m-patch14-224.
    Args:
    ...
    processor = SiQ_VLProcessor(
        tokenizer=AutoTokenizer.from_pretrained(pretrained_text_model_path),
...
def get_stage2_model_and_processor(
    stage_1_checkpoint_path: str = "outputs/stage1/final",
    use_lora: bool = False,
    lora_r: int = 128,  # Rank of the update matrices (v2: 64 → 128)
    lora_alpha: int = 128,  # Alpha parameter for LoRA scaling (v2: 16 → 128, alpha/r=1.0)
