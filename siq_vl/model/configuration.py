...
class SiQ_VLTextConfig(Qwen3Config):
    """
    Configuration class for SiQ-VL text model.
    Updated: Use Qwen3Config instead of Qwen2Config for Qwen3-0.6B compatibility.
    Qwen3 has q_norm/k_norm (per-head RMSNorm) and no attention bias,
    while Qwen2 has attention bias and no q_norm/k_norm.
    """
...
class SiQ_VLProjectorConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        vision_hidden_size: int = 1152,  # default hidden size of SigLIP2-so400m
        text_hidden_size: int = 1024,  # default hidden size of Qwen3-0.6B
        hidden_act: str = "silu",  # default activation function of Qwen3-0.6B
        *args,
        **kwargs,
    ):
        """
...
if __name__ == "__main__":
    config = get_siq_vl_config("Qwen/Qwen3-0.6B", "google/siglip2-so400m-patch14-224")
    print(config)
